...
const UpdateCategoryModal = () => {
  ...
  const category = state.visibleUpdateCategoryModal;
  ...
  const [
    updateCategory,
    { isLoading: isLoadingUpdateCategory, isSuccess: isSuccessUpdateCategory },
  ...
  useEffect(() => {
    if (state.visibleUpdateCategoryModal) {
      setForm({
        name: category.name,
  ...
  }, [state.visibleUpdateCategoryModal]);
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      if (validateString(form.name) || validateString(form.description)) {
        const data = new FormData();
        selectedFile && data.append('image', selectedFile as File);
        data.append('name', form.name);
        data.append('description', form.description);
        await updateCategory({ id: category._id, body: data });
      } else {
        setErrValidate(true);
      }
    },
  ...
  const closeModal = useCallback(() => {
    ...
    setVisibleModal('visibleUpdateCategoryModal');
    ...
    <Modal>
      <section
        style={{
          ...
          transform: state.visibleUpdateCategoryModal
            ? 'translateX(0)'
            : 'translateX(100%)',
        }}
        ...
        >
          <div className='bg-white dark:bg-darkBlue p-6 flex justify-between items-center'>
            <div className='flex flex-col gap-[8px]'>
              <p className='text-xl'>{t('update_category')}</p>
              <p className='text-sm'>{t('mess_update_category')}</p>
            </div>
            <button
